...
(function () {
    ...
    _.extend(bbGrid.RowView.prototype, Backbone.View.prototype, {
        ...
        setSelection: function (options) {
            ...
            if (this.selected || options.isShown) {
				...
				//add by yss,del the unselected item
				//this.view.selectedRows.push(this.model.id);
				//find unselected arr id
				var iUnSelected = -1;
				for (var i = 0; i < this.view.selectedRows.length;i++){
					if (this.model.id === this.view.selectedRows[i]){
						iUnSelected = i;						
					}					
				}
				//del
				if(iUnSelected != -1){
					this.view.selectedRows.splice(iUnSelected,1);					
				}
			}
			else {
        ...
        template: _.template(
            '<div class="col-md-1 bbGrid-pager">\
                <ul class="nav nav-pills">\
                    ...
                    <li>\
                        <div class="bbGrid-page-counter pull-left"><%=dict.page%>&nbsp</div>\
            ...
            <% if (rowlist) {%>\
                <div class="bbGrid-pager-rowlist-label pull-left"><%=dict.rowsOnPage%>&nbsp</div>\
    ...
    bbGrid.View = function (options) {
        ...
        this.rowViews = {};		
        this.selectedRows = this.selectedRows || []; //alter by yss
        this.currPage = 1;
        if (!this.collection) {
